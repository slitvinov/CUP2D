/* mesh2iso.c — extract AMR isolines and render to PNG (no image buffer).
   Uses marching squares on dual AMR grid (from iso2d.c), streams PNG
   scanlines. Usage: ./mesh2iso -g vel.xyz.raw -s vel.vort.raw -o iso.png -n
   1024 -w 1 -a -36 -b 36 -d 6 */
#include <math.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
static int findActual(struct Cell *cells, long long ncell, struct Cell *result,
                      struct vec2i lower, int level) {
  uint64_t target = morton_code(lower.x, lower.y);
  long long lo = 0, hi = ncell;
  while (lo < hi) {
    long long mid = lo + (hi - lo) / 2;
    if (cells[mid].morton < target) lo = mid + 1;
    else
      hi = mid;
  }
  ...
  *result = cells[lo];
  ...
  if (vec2i_eq(vec2i_shr(result->lower, f), vec2i_shr(lower, f))) return 1;
  if (lo > 0) {
    *result = cells[lo - 1];
...
static void png_chunk(FILE *f, const char *type, const unsigned char *d,
                      unsigned n) {
  unsigned char h[8];
  put32(h, n);
  memcpy(h + 4, type, 4);
...
int main(int argc, char **argv) {
  if (argc < 11) {
    fprintf(stderr,
            "usage: %s -g coords.xyz.raw -s scalar.raw -o out.png"
            " -n size -w linewidth\n"
  ...
  if (sc_sz == (long)(ncell * sizeof(double))) {
    ...
    fprintf(stderr,
            "mesh2iso: error: scalar size %ld does not match %lld cells\n",
            sc_sz, ncell);
    return 1;
  }
  fclose(fp);
  ...
  for (int y = 0; y < N; y++) {
    row[0] = 0; /* filter: none */
    for (int x = 0; x < N; x++) row[1 + x] = fb[y * N + x] ? 0 : 255;
    z.next_in = row;
    z.avail_in = N + 1;

...
public:
  ...
  void load0(Real *p0, Real *block[3][3][4], TreeState nei[3][3],
        ...
        if (info->level > 0 && use_averages) {
          ...
          if (cond) {
            ...
            if (myblocks[icode] != nullptr) {
              ...
              int bytes = (e[0] - s[0]) * dim * sizeof(Real);
              if (bytes) {
                int start[2] = {s[0] + std::max(cx, 0) * (_BS_ / 2) -
                                    cx * _BS_ + std::min(0, cx) * (e[0] - s[0]),
                                s[1] + std::max(cy, 0) * (_BS_ / 2) -
                                    cy * _BS_ +
    ...
    for (int ii = 0; ii < coarsened_nei_codes_size; ++ii) {
      ...
      if (!stencil.tensorial && !use_averages && abs(cx) + abs(cy) > 1)
        continue;
      int s[2] = {cx < 1 ? (cx < 0 ? stencil.sx : 0) : _BS_,
                  cy < 1 ? (cy < 0 ? stencil.sy : 0) : _BS_};
      int e[2] = {cx < 1 ? (cx < 0 ? 0 : _BS_) : _BS_ + stencil.ex - 1,
                  cy < 1 ? (cy < 0 ? 0 : _BS_) : _BS_ + stencil.ey - 1};
      int sC[2] = {cx < 1 ? (cx < 0 ? ((stencil.sx - 1) / 2) : 0) : (_BS_ / 2),
                   cy < 1 ? (cy < 0 ? ((stencil.sy - 1) / 2) : 0) : (_BS_ / 2)};
      int bytes = (e[0] - s[0]) * dim * sizeof(Real);
      if (!bytes)
        continue;
      if (use_averages) {
  ...
  void load(std::unordered_map<long long, TreeState> *tree,
    ...
    for (int icode = 0; icode < 9; icode++) {
      ...
      case ChildrenAreActive:
        long long id = sim.levels[info->level] + info->Zchild[1 + cx][1 + cy];
        Info *nn = (*all)[id];
        blocks[1 + cx][1 + cy][0] =
            (*all)[sim.levels[info->level + 1] + nn->Zchild[0][0]]->block;
        blocks[1 + cx][1 + cy][1] =
            (*all)[sim.levels[info->level + 1] + nn->Zchild[0][1]]->block;
        blocks[1 + cx][1 + cy][2] =
            (*all)[sim.levels[info->level + 1] + nn->Zchild[1][0]]->block;
        blocks[1 + cx][1 + cy][3] =
	(*all)[sim.levels[info->level + 1] + nn->Zchild[1][1]]->block;
        break;
      }
    }

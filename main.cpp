  ...
  void CoarseFineInterpolation(const BlockInfo &info) {
	...
	for (int iy = s[1]; iy < e[1]; iy += 2) {
	  ...
	  for (int ix = s[0]; ix < e[0]; ix += 2) {
		...
		dudy = 0.5 * (c[XX + nc[0] * (YY + 1)] -
			      c[XX + nc[0] * (YY - 1)]);
		dudy2 = (c[XX + nc[0] * (YY + 1)] +
			 c[XX + nc[0] * (YY - 1)]) -
			2.0 * c[XX + nc[0] * (YY)];
	      }
		...
		dudx = 0.5 * (c[XX + 1 + nc[0] * (YY)] -
			      c[XX - 1 + nc[0] * (YY)]);
		dudx2 = (c[XX + 1 + nc[0] * (YY)] +
			 c[XX - 1 + nc[0] * (YY)]) -
			2.0 * c[XX + nc[0] * (YY)];
	      }
	      m[ix - start[0] + nm[0] * (iy - start[1])] =
...
struct ScalarLab : public BlockLab<ScalarElement> {
  ...
  template <int dir, int side> void Neumann2D(bool coarse) {
	...
	cb[ix - stenBeg[0] + n[0] * (iy - stenBeg[1])] =
	    cb[(dir == 0 ? (side == 0 ? 0 : bsize[0] - 1) : ix) -
		  stenBeg[0] +
